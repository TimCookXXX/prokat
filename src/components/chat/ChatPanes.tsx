...
//
// А ПОЛ нужен телефону, повёрнутому на бок. Там ширина уже «десктопная» (сайдбар
// появляется с md), а высота — около 390px: разность давала 74px, и переписка
// схлопывалась в полоску, где не помещались ни строка списка, ни поле ввода.
// Обещание «панель помещается без прокрутки страницы» на такой высоте выполнить
// нечем — обложка с героем занимают 292px сами по себе, — и честный ответ здесь
// прокрутка, а не нерабочая панель.
const PANEL = "md:h-[max(20rem,min(calc(100svh-var(--account-hero-block)-1.5rem),40rem))]";
