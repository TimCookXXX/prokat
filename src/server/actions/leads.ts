...
const revealSchema = z.object({
  ...
  tab: z.enum(["cheapest", "noMoneyDeposit", "sameDay"]).optional(),
...
export async function revealShopPhone(input: unknown): Promise<LeadResult<{ phone: string; display: string }>> {
  const parsed = revealSchema.safeParse(input);
  if (!parsed.success) return { ok: false, error: "invalid_input" };
  const v = parsed.data;
  const who = await visitor();
  const limit = checkLimit(who.sessionId ?? who.ip, "lead");
  if (!limit.ok) return { ok: false, error: "rate_limited" };

  const db = getDb();
  let shop: { id: string; phone: string | null } | undefined;
  let itemClassId: string | null = null;
  if (v.offerId) {
    const [row] = await db
      .select({ shopId: rentalShops.id, phone: rentalShops.phone, itemClassId: offers.itemClassId })
      .from(offers)
      ...
      .limit(1);
    if (row) {
      shop = { id: row.shopId, phone: row.phone };
      itemClassId = row.itemClassId;
    }
  } else {
    [shop] = await db.select({ id: rentalShops.id, phone: rentalShops.phone }).from(rentalShops)
  ...
  return { ok: true, data: { phone: shop.phone, display: formatPhone(shop.phone) } };
}
...
async function offerSignal(input: unknown, type: LeadType): Promise<LeadResult> {
  ...
  if (!parsed.success) return { ok: false, error: "invalid_input" };
  const who = await visitor();
  if (!checkLimit(who.sessionId ?? who.ip, "lead").ok) return { ok: false, error: "rate_limited" };
...
export async function createRegularRequest(input: unknown): Promise<LeadResult> {
  ...
  const v = parsed.data;
  const who = await visitor();
  const limit = checkLimit(who.sessionId ?? who.ip, "lead_form");
  if (!limit.ok) return { ok: false, error: "Слишком часто. Попробуйте через минуту." };
