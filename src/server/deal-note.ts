...
export async function writeDealNote(tx: Tx, note: DealNote): Promise<void> {
   ...
   *
   * greatest, а не голое now(): now() — это время НАЧАЛА транзакции. Внутри
   * длинной (бан с полусотней записей, подтверждение с конкурентами) собеседник
   * успевает отправить реплику и закоммитить, и отметка уехала бы назад — тред
   * провалился бы в списке ниже собственного последнего сообщения. Тот же
   * приём и по той же причине стоит в insertMessage. */
  await tx.update(chatThreads)
    .set({ lastMessageAt: sql`greatest(${chatThreads.lastMessageAt}, now())` })
