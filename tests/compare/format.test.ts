...
describe("buildFaq", () => {
  ...
  it("answers from fresh offers only", () => {
    ...
    expect(faq[0]).toEqual({
      ...
      a: "Суточная цена — от 500 ₽ у 2 прокатов в Краснодаре. Итог за 1 сутки — от 500 ₽: мы сразу учитываем минимальный срок и недельный тариф проката.",
    });
    expect(faq[1].a).toContain("Без денежного залога — 1 из 2 предложений (часть просит паспорт).");
    expect(faq[2].q).toBe("Как выбрать прокат поближе?");
    expect(faq[2].a).toContain("Доставка есть у 2 прокатов");
  });
